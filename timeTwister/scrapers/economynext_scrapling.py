...
def _fetch_bytes_http_fallback(
    ...
    *,
    ...
    """curl_cffi then requests — GHA datacenter IPs often get CF 202 from Scrapling Fetcher."""
...
def fetch_page(
    ...
    *,
    ...
    if last_err:
        print(f"[WARN] All Scrapling Fetcher profiles failed for {url[:70]}: {last_err}")
    return None
...
def fetch_bytes(
    ...
    *,
    ...
    """Raw response body (RSS/XML or JSON API). Scrapling first, curl_cffi on GHA."""
    if params:
        sep = "&" if "?" in url else "?"
...
def fetch_text(
    ...
    *,
    ...
    """HTML text for article/list pages."""
    page = fetch_page(url, params=params, accept=accept, timeout=timeout)
    if page:
